def count_values(data):
    print('DATA COUNT:\n')
    print(data.all_values.count())


def count_column_values(data):
    print('Values per column:\n')
    for column in data.columns:
        print(data.all_values[column].value_counts(), 'n')


def get_math_stats(data):
    print('\nStatistics from num data:\n')
    for column in set(data.all_values.describe().columns):
        print('\n' + column, '\n')
        col_data = data.all_values[column]
